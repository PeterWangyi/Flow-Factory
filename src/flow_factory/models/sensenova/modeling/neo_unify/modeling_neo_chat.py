...
class NEOChatModel(PreTrainedModel):
    ...
    def __init__(
        self, config: NEOChatConfig, vision_model=None, language_model=None, use_flash_attn=True
    ):
    ...
    def _generate_think(
        ...
        for _ in range(max_think_tokens):
            ...
            self.language_model.model.current_index = t_idx
            outputs = self.language_model(
                input_ids=next_token.unsqueeze(0), past_key_values=past_key_values, use_cache=True
            )
            past_key_values = outputs.past_key_values
            t_idx += 1
    ...
    def interleave_gen_image_only(
        ...
        for i, part in enumerate(parts):
            if len(part) > 0:
                ...
                part_ids = tokenizer(part, return_tensors="pt", add_special_tokens=False)[
                    "input_ids"
                ].to(device)
            ...
            if i < len(parts) - 1:
                ...
                if self.noise_scale_mode in ("resolution", "dynamic", "dynamic_sqrt"):
                    noise_scale = math.sqrt(
                        (grid_h * grid_w) / (merge_size**2) / self.noise_scale_base_image_seq_len
                    )
                    base = float(self.noise_scale_base_image_seq_len)
                    noise_scale = math.sqrt((grid_h * grid_w) / (merge_size**2) / base) * float(
                        self.noise_scale
                    )
                ...
                for step_i in step_iter:
                    ...
                    image_prediction = self.unpatchify(
                        z, self.patch_size * merge_size, cur_image_size[1], cur_image_size[0]
                    )
    ...
    def it2i_generate(
        ...
        query_condition = self._build_t2i_query(
            question_condition, system_message=SYSTEM_MESSAGE_FOR_GEN, append_text=think_content
        )
        query_img_condition = (
        ...
        query_uncondition = (
            self._build_t2i_query("", append_text=IMG_START_TOKEN) if needs_uncondition else None
        )
        ...
        if query_img_condition is not None:
            ...
            ) = self._build_it2i_inputs(tokenizer, query_img_condition, pixel_values, grid_hw)
        else:
        ...
        for layer_idx in range(len(past_key_values_condition.layers)):
            past_key_values_condition.layers[layer_idx].keys = past_key_values_condition.layers[
                layer_idx
            ].keys.expand(batch_size, *past_key_values_condition.layers[layer_idx].keys.shape[1:])
            ...
            if past_key_values_img_condition is not None:
                ...
                past_key_values_img_condition.layers[
                    layer_idx
                ].values = past_key_values_img_condition.layers[layer_idx].values.expand(
    ...
    def t2i_generate(
        ...
        query_uncondition = (
            self._build_t2i_query("", append_text=IMG_START_TOKEN) if needs_cfg else None
        )
        ...
        if query_uncondition is not None:
            input_ids_uncondition, indexes_uncondition, attention_mask_uncondition_prefix = (
        ...
        indexes_image_condition = self._build_t2i_image_indexes(
            token_h, token_w, indexes_condition.shape[1], device=input_ids_condition.device
        )
        indexes_image_uncondition = (
            self._build_t2i_image_indexes(
                token_h, token_w, indexes_uncondition.shape[1], device=input_ids_uncondition.device
            )
            if indexes_uncondition is not None
            else None
        )
