...
def set_attn_backend(backend: str) -> str:
    ...
    if backend not in _VALID_ATTN_BACKENDS:
        raise ValueError(
            f"Unknown attention backend {backend!r}. " f"Expected one of {_VALID_ATTN_BACKENDS}."
        )
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if exist_non_image_gen_tokens and not exist_image_gen_tokens:
            return self.forward_und(
                hidden_states, indexes, attention_mask, past_key_values, cache_position, **kwargs
            )
