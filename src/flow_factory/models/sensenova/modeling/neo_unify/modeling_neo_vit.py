...
def precompute_rope_freqs_sincos(dim: int, max_position: int, base: float = 10000.0, device=None):
    """Precompute one-dimensional RoPE cosine and sine values."""
...
def apply_rotary_emb_1d(
    ...
    """Apply one-dimensional RoPE to a slice of the input tensor."""
    # x: (..., seq_len, dim_part)
    # positions: (..., seq_len)
...
def apply_2d_rotary_pos_emb(
    ...
    """Apply two-dimensional RoPE to the input tensor."""
    dim = x.shape[-1]
    dim_half = dim // 2

    # Use the first half of the embedding for one axis and the second half for the other.
    x_part_1 = x[..., :dim_half]
    x_part_2 = x[..., dim_half:]

    # Apply the rotation associated with abs_positions_x to x_part_1.
    rotated_part_1 = apply_rotary_emb_1d(x_part_1, cos_cached_x, sin_cached_x, abs_positions_x)
    # Apply the rotation associated with abs_positions_y to x_part_2.
    rotated_part_2 = apply_rotary_emb_1d(x_part_2, cos_cached_y, sin_cached_y, abs_positions_y)

    # Concatenate the rotated parts while preserving their axis order.
...
class NEOVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, grid_hw=None) -> torch.Tensor:
        ...
        for i in range(grid_hw.shape[0]):
            h, w = grid_hw[i]
            patches_per_img = (
                patch_embeds[cur_position : cur_position + h * w].view(h, w, -1).unsqueeze(0)
            )
